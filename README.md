...
- Solana `getSlot`에서 HTML이 내려옴
  - `SOLANA_RPC_URL`에 “웹사이트 URL”이 들어간 상태입니다. JSON-RPC 엔드포인트로 교체하세요.
- Solana `getBlock: Block not available for slot`
  - skipped slot이거나 RPC 노드가 해당 블록을 보관하지 않는 경우입니다.
